/*
  ...
  SDL2 port for ArcaOS/OS2 by OS2World, 2026.
*/
...
/* Three copies of the background, guaranteed adjacent in a single buffer.
   The water render functions index into BkGdImage with offsets that
   intentionally overflow into the Pre/Post regions; they must be contiguous.
 */
static byte BkGdBuffer[3 * WATERWID * WATERHGT];
#define BkGdImagePre  (BkGdBuffer)
#define BkGdImage     (BkGdBuffer + WATERWID * WATERHGT)
#define BkGdImagePost (BkGdBuffer + 2 * WATERWID * WATERHGT)
...
void water(SDL_Surface *image, SDL_Window *window);
...
int main(int argc, char **argv)
{
  ...
  /* Fill the extra palette entries with white and set the image palette */
  {
    int nwhite;

    nwhite = 256 - palette->ncolors;
    memset(&palette->colors[palette->ncolors], 255, nwhite * sizeof(SDL_Color));
    SDL_SetPaletteColors(image->format->palette, palette->colors, 0, 256);
  }

  screen = SDL_GetWindowSurface(window);
  (void)screen; /* used via SDL_GetWindowSurface inside water() */
...
/* Each bit of the mode variable is used to represent the state of a
  ...
  /*  The surfer (2) ... Swirling effect */
    ...
    /* Blit the modified image surface to the window surface and show it */
    frames++;
    {
      SDL_Surface *ws = SDL_GetWindowSurface(window);
      SDL_BlitSurface(image, NULL, ws, NULL);
